...
public:
    bool canPartitionKSubsets(vector<int>& nums, int k) {
        ...
        vector<bool> used(nums.size(), false);          //记录数字使用情况
        vector<int> bucket(k, 0);      //将数字分到k个桶内
        return helper(bucket, 0, nums, 0, used, target);
    }
    //start 记录当前要装的桶，遍历到哪个数字了。  index为当前要装的桶
    bool helper(vector<int>& bucket, int index, vector<int>& nums, int start, vector<bool>& used, int target) {
        if (index == bucket.size()) return true;
        if (bucket[index] == target) return helper(bucket, index + 1, nums, 0, used, target);   //装填下一个桶
        for (int i = start; i < nums.size(); i++) {
            if (used[i]) continue;  //剪掉使用过的数字
            if (bucket[index] + nums[i] > target) continue; //剪掉相加大于target
            used[i] = true;
            bucket[index] += nums[i];
            if (helper(bucket, index, nums, start, used, target)) {
                return true;
            }
            bucket[index] -= nums[i];
            used[i] = false;
            if (bucket[index] == 0) return false;         //说明当前桶装填全部失败，因为不同桶没有顺序差别，所有直接返回false
        }
        return false;
    }
};
